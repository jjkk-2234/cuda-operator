...
def solve_triton_naive(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, M: int, N: int):
    BLOCK_SIZE = 1024
    n_elements = M * N
...
def solve_triton_1d(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, M: int, N: int):
    n_elements = M * N
...
@triton.jit
def matrix_add_kernel_2d(a_ptr, b_ptr, c_ptr, M, N, BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr):
...
def solve_triton_2d(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, M: int, N: int):
    grid = lambda meta: (
        (triton.cdiv(M, meta['BLOCK_M']), triton.cdiv(N, meta['BLOCK_N']))
    )
    matrix_add_kernel_2d[grid](a, b, c, M, N)

def benchmark(func, A, B, C, M, N, warmup=10, repeat=100):
    # Warmup
...
def main():
    ...
    # ---------- 正确性校验：非对称 / 非整除等任意形状 ----------
    shapes = [(8192, 8192), (1000, 999), (4096, 100), (1, 4096), (3, 5)]
    solvers = [("pytorch", matrix_add_pytorch), ("naive", solve_triton_naive),
               ("1d", solve_triton_1d), ("2d", solve_triton_2d)]
    for (M, N) in shapes:
        ...
        for name, solve in solvers:
            ...
            print(f"[{name}] shape={tuple(A.shape)}: allclose={ok}")
            assert ok, f"{name} 在 shape={tuple(A.shape)} 下校验失败"
    print("正确性校验：ALL PASS\n")

    # ---------- 性能测试（与 vector_add 同数据量：2^26 个元素 / 每矩阵 268MB） ----------
    M = N = 8192
    A = torch.randn((M, N), device=device, dtype=torch.float32)
    B = torch.randn((M, N), device=device, dtype=torch.float32)
    ...
    print("开始性能测试 (预热 10 次，计时 100 次取平均)...\n")

    time_pytorch = benchmark(matrix_add_pytorch, A, B, C_pytorch, M, N)
    time_triton_naive = benchmark(solve_triton_naive, A, B, C_triton_naive, M, N)
    time_triton_1d = benchmark(solve_triton_1d, A, B, C_triton_1d, M, N)
    time_triton_2d = benchmark(solve_triton_2d, A, B, C_triton_2d, M, N)
    ...
    total_bytes = 3 * M * N * bytes_per_element  # A读 + B读 + C写
